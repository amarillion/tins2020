...
public:
	...
	DEPRECATED virtual void Add(const char *item, int msg) { addItem(item, msg); }
	DEPRECATED virtual void Add() { addSeparator(); }; // insert spacer;
	DEPRECATED virtual void Add(const char *submenu, std::shared_ptr<Menu> menu) { addItem(submenu, menu); }
